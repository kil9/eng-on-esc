...
├── Cargo.lock
├── build.rs            # CARGO_PKG_VERSION 으로 ICON+VERSIONINFO 통합 .rc 를 OUT_DIR 에 생성, embed-resource 로 컴파일·링크
├── src/
...
├── .github/
│   └── workflows/
│       └── release.yml # v* 태그 push 시 windows-latest 에서 빌드 → unsigned exe + SHA256SUMS.txt 를 GitHub Releases 에 업로드
├── target/             # 빌드 산출물 (커밋 제외)
├── README.md
