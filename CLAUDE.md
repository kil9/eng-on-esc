...
```
.
├── Cargo.toml          # Rust 프로젝트 정의 (windows crate 0.58 + build-dep embed-resource)
├── Cargo.lock
├── build.rs            # embed-resource 로 assets/icon.rc 를 .res 컴파일·링크
├── src/
│   └── main.rs         # 단일 엔트리포인트 — 키보드 후킹, 트레이 아이콘, IME 제어 모두 여기
├── assets/
│   ├── icon.ico        # 트레이 아이콘 (멀티 사이즈 16/24/32/48/64/256, exe 에 임베드)
│   └── icon.rc         # ICON 리소스 정의 (`1 ICON "icon.ico"`)
├── tools/
│   └── generate-icon.ps1   # icon.ico 재생성 스크립트 (System.Drawing 기반)
├── target/             # 빌드 산출물 (커밋 제외)
├── README.md
├── AGENTS.md           # 이 문서
...
- 아이콘은 `build.rs` + `embed-resource` 가 `assets/icon.rc` 를 컴파일해 exe 에 임베드한다 (`1 ICON "icon.ico"`). 런타임에는 `LoadIconW(hinst, MAKEINTRESOURCE(IDI_TRAY_ICON=1))` 로 로드. 실패 시 `LoadIconW(IDI_APPLICATION)` 으로 폴백.
- 아이콘 디자인은 ESC 키 캡 모양. `tools/generate-icon.ps1` 로 재생성 가능 (System.Drawing). 16/24/32/48/64/256 PX 멀티 사이즈 PNG 를 ICO 컨테이너에 패킹.
- 아이콘을 수정한 뒤에는 `cargo build` 가 `build.rs` 를 다시 돌리도록 `assets/icon.ico` 또는 `assets/icon.rc` 의 mtime 만 바꾸면 충분 (embed-resource 가 의존성으로 추적).
...
- `embed-resource = 2` (build-dependency) — `assets/icon.rc` 를 MSVC `rc.exe` / GNU `windres` 로 컴파일해 .res/.coff 형태로 링커에 넘겨 exe 에 리소스 임베드. MSVC 툴체인을 쓸 경우 Visual Studio Build Tools 의 C++ 도구셋이 필요.
