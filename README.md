...
미리 빌드된 바이너리는 [GitHub Releases](https://github.com/kil9/eng-on-esc/releases) 에서 받을 수 있다. 별도 설치 과정 없이 `eng-on-esc.exe` 단독 실행이면 충분하다.
...
빌드 산출물: `target\release\eng-on-esc.exe`
...
```
target\release\eng-on-esc.exe
```

### 트레이 아이콘 (icon.ico)

전용 아이콘은 `assets/icon.ico` 에 들어 있고 빌드 시 `embed-resource` 로 exe 안에 임베드된다. 별도로 `.ico` 파일을 같이 배포할 필요 없이 `eng-on-esc.exe` 단독으로 실행 가능하다.
