...
- ✅ M1 전용 트레이 아이콘 (ESC 키 캡 모양, 멀티 사이즈 ICO, 런타임 LoadImageW 로딩)
...
- **날짜**: 2026-05-08
- **결정**: `embed-resource` 크레이트로 리소스 컴파일 임베드 대신, `assets/icon.ico` 를 별도 파일로 두고 런타임 `LoadImageW(LR_LOADFROMFILE)` 로 로딩한다.
- **이유**:
  - 사용자 빌드 환경에 MSVC `rc.exe` / MinGW `windres` 가 없어 `embed-resource` 가 동작하지 않음. 추가 툴체인 설치를 강요하지 않으면서 전용 아이콘을 도입할 수 있는 가장 가벼운 길.
  - 외부 파일 의존이 단점이지만, 검색 후보 경로(① exe 옆, ② `<exe>/../../assets/icon.ico` dev 폴백, ③ 시스템 기본 아이콘)로 dev/배포 양쪽에서 무리 없이 동작.
- **대안**:
  - `embed-resource` 임베드 — rc/windres 필수, 환경 추가 설치 비용 발생.
  - `include_bytes!` + `CreateIconFromResourceEx` 로 메모리 내 ICO 파싱 — 깔끔하지만 변환 오버헤드와 `RT_GROUP_ICON` 포맷 직접 다룸.
...
- 2026-05-08: M1 완료. `tools/generate-icon.ps1` 으로 ESC 키 캡 모양 멀티 사이즈 `assets/icon.ico` 생성. `main.rs` 의 트레이 아이콘 로딩을 `LoadIconW(IDI_APPLICATION)` 에서 `LoadImageW(LR_LOADFROMFILE)` 로 변경, 시스템 기본 아이콘 폴백 유지. (개발 환경에 C++ 툴체인 부재로 build-time 검증은 스킵. 사용자 환경에서 `cargo build --release` 통과 후 트레이에 새 아이콘 표시 여부 확인 필요.)
