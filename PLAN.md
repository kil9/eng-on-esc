...
핵심 동작이 thread별 자체 상태머신 기반으로 안정화되었다. 사용자 환경(Windows 11 Microsoft Korean IME)에서 메모장/터미널/브라우저를 오가며 동작 검증 완료. 전용 트레이 아이콘 도입(M1) 완료.
...
- [ ] `assets/icon.rc` 에 `VERSIONINFO` 블록 추가 — 제품명/파일 설명/회사·저작권/파일 버전·제품 버전. Windows 파일 속성 창에서 식별 가능.
- [ ] `Cargo.toml` 의 `version` 과 `VERSIONINFO` 버전을 일치시키는 방법 결정 (간단히는 수동 동기화, 추후 build.rs 에서 `CARGO_PKG_VERSION` 으로 자동 생성하는 옵션 검토).
- [ ] `CHANGELOG.md` 신설, v0.1.0 항목 작성 ("Keep a Changelog" 양식 권장).
- [ ] `.github/workflows/release.yml` 작성:
  - 트리거: `v*` 태그 push (예: `v0.1.0`)
  - runner: `windows-latest` (MSVC 툴체인 기본 제공)
  - 단계: rust toolchain 설치 → `cargo build --release` → `target\release\esc-eng-ime.exe` 와 `SHA256SUMS.txt` 생성 → `softprops/action-gh-release` 등으로 Releases 업로드
- [ ] `README.md` 에 다운로드/설치 섹션 추가 — Releases 링크, "처음 실행 시 SmartScreen 보호 경고가 뜨면 '추가 정보 → 실행' 으로 진행 (unsigned 바이너리 한계)" 안내, 옵션으로 SHA256 검증 명령.
- [ ] v0.1.0 태그 발행 후 워크플로 동작 확인. Releases 페이지에서 다운로드한 exe 가 새 머신(또는 다른 사용자 계정)에서도 정상 실행되는지 종단 검증.

### M1.5. (선택) 디자인 결정: 버전 동기화

`Cargo.toml` 과 `assets/icon.rc` 의 버전 문자열을 어디서 단일 소스로 둘지, 빌드 시 자동화할지를 M0 진행 중에 정한다. 후보:

- 수동 동기화 (간단, 실수 가능)
- `build.rs` 에서 `CARGO_PKG_VERSION` 을 읽어 `OUT_DIR` 에 임시 .rc 작성 후 `embed-resource` 로 컴파일 (자동, 한 번만 셋업)
...
- 현재(릴리즈 전): 로컬 `cargo build --release` 후 `target\release\esc-eng-ime.exe` 직접 사용.
