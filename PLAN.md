...
macOS 버전: Step 1–3 + Step 4 대부분 완료. `rdev` 기반 키보드 후킹, `tray-icon + tao` 메뉴바, LaunchAgent 자동 시작 구현. README/CHANGELOG 갱신 완료. 남은 것: 수동 동작 확인 + v0.4.0 태그 발행.
...
- [x] macOS 에 Rust 설치 (완료)
- [x] `macos/` 에서 `cargo check` / `cargo clippy` 통과 확인 — `rdev` `unstable_grab` 피처 추가로 해결
- [ ] `cargo run` 후 손쉬운 사용 권한 허용 → ESC 입력 시 Caps Lock 토글 여부 확인
- [ ] 상태 어긋남 시나리오 테스트: 앱 시작 후 한국어 모드에서 ESC (첫 번째는 무동작이 정상)

**Step 2 — 메뉴바 아이콘 구현** ✅ 완료

구조:
```
main()
  ├─ thread::spawn → grab(keyboard_callback)   // 백그라운드 스레드
  ├─ thread::spawn → worker (ESC 디바운스)
  └─ EventLoop::new().run(...)                  // 메인 스레드 점유
```

추가된 의존성 (`macos/Cargo.toml`):
```toml
tray-icon = "0.19"   # features=["tao"] 는 0.19 에서 제거됨
tao = "0.30"         # features=["macos"] 도 불필요 (플랫폼 자동 검출)
```

- [x] `macos/Cargo.toml` 에 `tray-icon = "0.19"`, `tao = "0.30"` 추가
- [x] `grab` 을 별도 스레드로 이동, 메인 스레드에서 `EventLoop::run`
- [x] `TrayIconBuilder` 로 메뉴바 아이콘 + 메뉴 구성 (CheckMenuItem, PredefinedMenuItem::separator, MenuItem)
- [x] 종료 메뉴 → `ControlFlow::Exit`
- [x] `EventLoopExtMacOS::set_activation_policy(ActivationPolicy::Accessory)` — Dock 아이콘 숨김

**Step 3 — 자동 시작** ✅ 완료

plist 경로: `~/Library/LaunchAgents/io.github.kil9.eng-on-esc.plist`

- [x] `autostart_is_enabled()` — plist 파일 존재 여부 확인
- [x] `autostart_enable()` — `std::env::current_exe()` 로 현재 바이너리 경로로 plist 생성
- [x] `autostart_disable()` — plist 파일 삭제
- [x] 메뉴 토글 연결 + `CheckMenuItem::set_checked()` 로 체크 표시 갱신

**Step 4 — 마무리**

- [ ] macOS 앱 아이콘 개선 (현재: 22×22 원형 dot 코드 생성. 추후 ESC 키 캡 디자인 PNG → `.icns` 변환 검토)
- [ ] `cargo run` 후 Gatekeeper + 손쉬운 사용 권한 허용 → ESC 동작 수동 확인
- [x] README 에 macOS 설치/사용 방법 추가 (Gatekeeper + 손쉬운 사용 권한 안내 포함)
- [x] `CHANGELOG.md` v0.4.0 항목 추가
- [ ] GitHub Releases 에 `eng-on-esc-macos` 바이너리 포함 v0.4.0 태그 발행
