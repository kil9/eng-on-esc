...
- [x] `macos/` 독립 Rust 프로젝트 생성
- [x] `rdev` 기반 글로벌 키보드 후킹 + 자체 상태 머신 스켈레톤 작성
- [x] GitHub Actions 워크플로 macOS 빌드 job 추가 (`macos-latest`)
- [ ] Accessibility 권한 안내 / 자동 권한 요청 UI
- [ ] 메뉴바 상주 아이콘 (`tray-icon` + `tao`, `NSStatusItem`)
- [ ] 자동 시작 (`~/Library/LaunchAgents/` plist 생성/삭제)
- [ ] macOS 동작 검증 (한국어 IME 환경에서 Caps Lock 토글 확인)
- [ ] README macOS 섹션 추가
