...
### M1. modern IME 환경에서 OS API 재검증 (검토 진행 중)

‘이전 버전의 Microsoft IME’ 토글이 ON 상태에서 IMC_SETOPENSTATUS 등을 시도했었는데, OFF(modern) 상태에서 다시 검증할 필요가 있다. 자체 상태 머신은 마우스/`Win+Space` 토글을 못 따라가므로, OS 가 제공하는 현재 모드 조회 API 가 modern 에서 정상 응답한다면 그것을 일차 소스로 삼는 게 더 안전하다.

- [ ] modern IME 환경에서 `IMC_GETOPENSTATUS` 응답 신뢰성 확인
- [ ] `AttachThreadInput` + `ImmGetContext` 결과가 modern 에서 다른지 확인
- [ ] TSF API(`ITfThreadMgr` 등) 시도 가능성 검토
- [ ] 결과에 따라 자체 상태머신을 보조로만 두거나 제거
