...
macOS 버전: `macos/` 디렉토리에 스켈레톤 생성. `rdev` 기반 키보드 후킹 + 상태 머신 구조 작성. 트레이/자동 시작 미구현(TODO).
...
| `main` (메인 스레드) | `grab(keyboard_callback)` 실행 (트레이 구현 후 별도 스레드로 이동 예정). |
...
- [x] `macos/` 독립 Rust 프로젝트 생성 (`macos/Cargo.toml`, `macos/src/main.rs`)
- [x] `rdev = "0.5"` 기반 글로벌 키보드 후킹 + Caps Lock 자체 상태 머신 스켈레톤
- [x] GitHub Actions 워크플로 `build-macos` job 추가 (`macos-latest`, `working-directory: macos`)

#### 다음 작업 순서

**Step 1 — 기본 동작 검증** (macOS 환경 필요)
- [ ] macOS 에 Rust 설치: `curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh`
- [ ] `macos/` 에서 `cargo check` / `cargo clippy` 통과 확인
- [ ] `cargo run` 후 손쉬운 사용 권한 허용 → ESC 입력 시 Caps Lock 토글 여부 확인
- [ ] 상태 어긋남 시나리오 테스트: 앱 시작 후 한국어 모드에서 ESC (첫 번째는 무동작이 정상)

**Step 2 — 메뉴바 아이콘 구현**

현재 `grab(keyboard_callback)` 이 메인 스레드를 점유한다.
`tao` 이벤트 루프도 메인 스레드가 필요하므로 구조를 바꿔야 한다:

```
main()
  ├─ thread::spawn → grab(keyboard_callback)   // 백그라운드 스레드
  ├─ thread::spawn → worker (ESC 디바운스)      // 이미 있음
  └─ EventLoop::new().run(...)                  // 메인 스레드 점유
```

추가할 의존성 (`macos/Cargo.toml`):
```toml
tray-icon = { version = "0.19", features = ["tao"] }
tao = { version = "0.30", default-features = false, features = ["macos"] }
```

메뉴 항목: ① 자동 시작 토글 (체크), ② 구분선, ③ 종료

- [ ] `macos/Cargo.toml` 에 `tray-icon`, `tao` 추가
- [ ] `grab` 을 별도 스레드로 이동, 메인 스레드에서 `EventLoop::run`
- [ ] `TrayIconBuilder` 로 메뉴바 아이콘 + 메뉴 구성
- [ ] 종료 메뉴 → `EventLoopWindowTarget::exit()` 호출 + `grab` 스레드 종료
- [ ] `NSActivationPolicy::Accessory` 설정 (Dock 아이콘 숨김) — `tao` Info.plist 또는 `objc2` 직접 호출

**Step 3 — 자동 시작**

macOS 자동 시작은 `~/Library/LaunchAgents/` 에 plist 파일을 생성/삭제하는 방식.

plist 경로: `~/Library/LaunchAgents/io.github.kil9.eng-on-esc.plist`
...
<plist version="1.0">
<dict>
  <key>Label</key>           <string>io.github.kil9.eng-on-esc</string>
  <key>ProgramArguments</key><array><string>/path/to/eng-on-esc</string></array>
  <key>RunAtLoad</key>       <true/>
</dict>
</plist>
```

- [ ] `autostart_is_enabled()` — plist 파일 존재 여부 확인
- [ ] `autostart_enable()` — 현재 exe 경로로 plist 생성 (`std::env::current_exe()`)
- [ ] `autostart_disable()` — plist 파일 삭제
- [ ] 메뉴 토글 연결 + 체크 표시

**Step 4 — 마무리**

- [ ] macOS 앱 아이콘 생성 (`.icns` 또는 `assets/` 에 PNG, 메뉴바용 16×16/32×32 흑백 권장)
- [ ] `cargo build --release` + Gatekeeper 동작 확인 (unsigned 바이너리는 최초 실행 시 우클릭 → 열기 필요)
- [ ] README 에 macOS 설치/사용 방법 추가 (권한 안내 포함)
- [ ] `CHANGELOG.md` macOS 첫 릴리즈 항목 추가
- [ ] GitHub Releases 에 `eng-on-esc-macos` 바이너리 포함 태그 발행

#### 기술 메모 (새 세션에서 참고)

- **`rdev` Caps Lock 주의**: macOS 에서 Caps Lock 은 하드웨어 레벨 디바운스가 있어 빠른 연타 시 이벤트가 누락될 수 있다. 문제가 생기면 Caps Lock 대신 `TISSelectInputSource` C API 직접 호출로 입력 소스를 전환하는 방식 검토 (`core-foundation` 크레이트 필요).
- **`SENDING_CAPS` 타이밍**: `simulate` 후 callback 이 같은 스레드가 아닌 별도 스레드에서 실행되므로 `SeqCst` ordering 사용 중. 타이밍 이슈 발생 시 `sleep` 조정 또는 채널 기반 동기화로 교체.
- **Gatekeeper**: 코드 사이닝 없는 바이너리는 최초 실행 시 차단됨. 사용자가 우클릭 → 열기로 허용 가능. Apple Developer 계정 없이 배포할 경우 README 에 안내 필수.
- **메뉴바 전용 앱**: `tao` 에서 `NSApp.setActivationPolicy(.accessory)` 설정 방법이 버전마다 다름. 안 되면 `Info.plist` 에 `LSUIElement = YES` 추가 (`build.rs` 또는 별도 plist 파일로 관리).
- **`rdev` 버전**: `0.5.x` 사용. `grab` API: `fn grab(callback: impl Fn(Event) -> Option<Event>) -> Result<(), GrabError>` — 블로킹.
