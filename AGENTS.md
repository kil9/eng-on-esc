...
├── assets/
│   └── icon.ico        # 트레이 아이콘 (멀티 사이즈 16/24/32/48/64/256)
├── tools/
...
- 아이콘은 `LoadImageW(IMAGE_ICON, LR_LOADFROMFILE | LR_DEFAULTSIZE)` 로 런타임에 `icon.ico` 를 읽어 사용. 후보 경로 순서:
  1. `<exe_dir>/icon.ico` — 배포 시 권장 (exe 옆에 두고 같이 배포)
  2. `<exe_dir>/../../assets/icon.ico` — `cargo run` 처럼 `target/<profile>/` 안에서 실행될 때 dev 폴백
  3. 둘 다 실패하면 `LoadIconW(IDI_APPLICATION)` 으로 폴백 (Windows 기본 아이콘)
- 아이콘 디자인은 ESC 키 캡 모양. `tools/generate-icon.ps1` 으로 재생성 가능 (System.Drawing). 16/24/32/48/64/256 PX 멀티 사이즈 PNG 를 ICO 컨테이너에 패킹.
